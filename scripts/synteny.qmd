...
Попробуем ген "modA" в качестве центрирующего гена

Находим ген из окрестностей modA, продукт которого - "alkaline phosphatase D family protein" для всех штаммов
...
target_region <- gtf %>%
  filter(type == "gene" & gene == target_gene_name) %>%
  mutate(start_target = start, end_target = end)

# Определяем окрестность window_size оснований в каждую сторону от target
...
С помощью IGV браузера нашли ближайшие гены в окрестности KP05_RS10430, которые имеют названия: modA, modB, modC, ptsP, betT.

betT не подходит.

modA, modB, modC одинаково подходят для определения таргетного региона, но мы помним, что они лежат на другом стренде, что нехорошо.

ptsP подходит, но хуже, чем mod\*

В итоге используем modA.
...
final_result %>%
      select(start, end, gene_id, protein_id)
...
center_coordinates <- function(gtf_data, center_gene, window_size = 2000) {
...
Наверное лучше разбить на группы по длине гомологов.
...
# Функция для центрирования координат относительно указанного гена
center_coordinates <- function(gtf_data, center_gene, window_size = 20000) {
  center_gene_coords <- gtf_data %>%
    filter(type == "CDS", gene_id == center_gene) %>%
    dplyr::select(seqid, center_start = start, center_end = end)
